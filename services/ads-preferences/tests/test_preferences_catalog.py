...
def test_patch_label_keeps_bearer_and_auth_replaces(client: TestClient, user_token: str) -> None:
    ...
    assert rotated.json()["options"] == {"model-name": "glm-5.3"}
...
def test_patch_options_replaces_model_name(client: TestClient, user_token: str) -> None:
    ...
    patched = client.patch(
        f"/v1/models/{model_id}",
        json={"options": {"model-name": "glm-5.2"}},
        headers=_auth(user_token),
    )
    assert patched.status_code == 200
    assert patched.json()["options"] == {"model-name": "glm-5.2"}
...
def test_unsupported_names_cannot_be_created_or_patched(
    ...
    headers = _auth(user_token)
    unsupported = {**MODEL_BODY, "options": {"model-name": name}}
    ...
    assert (
        client.patch(
            f"/v1/models/{model_id}", json={"options": {"model-name": name}}, headers=headers
        ).status_code
        == 400
    )
