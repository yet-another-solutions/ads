...
from supervisor_helpers import TOKEN, DirectPolicyClient, sandbox_body
...
def api(
    ...
    app = create_app(_table(settings, upstream), policy_client, CollectingAuditSink())
...
def _post(
    ...
    headers = {**ACCEPT, **({"x-ads-run": run} if run else {})}
...
def _open_run(api: TestClient) -> str:
    response = api.post(
        "/supervisor/runs",
        json=sandbox_body(),
...
def test_a_permitted_call_reaches_the_server(api: TestClient, upstream: Upstream) -> None:
    run = _open_run(api)
    answer = _post(api, _call("read", {"filePath": WORKDIR_FILE}), run)
...
def test_a_refused_call_never_reaches_the_server(api: TestClient, upstream: Upstream) -> None:
    ...
    run = _open_run(api)
    answer = _post(api, _call("read", {"filePath": "/etc/shadow"}), run)
...
def test_a_refusal_speaks_the_agent_s_own_protocol(api: TestClient) -> None:
    ...
    run = _open_run(api)
    answer = _post(api, _call("read", {"filePath": "/etc/shadow"}, request_id=7), run)
...
def test_a_tool_nothing_binds_is_refused(api: TestClient, upstream: Upstream) -> None:
    run = _open_run(api)
    answer = _post(api, _call("telepathy", {"thought": "rm -rf /"}), run)
    assert upstream.seen == []
    assert "error" in answer
...
def test_a_credential_in_the_arguments_is_refused(api: TestClient, upstream: Upstream) -> None:
    run = _open_run(api)
    answer = _post(api, _call("webfetch", {"url": "mirror.interlab", "body": f"K={AWS_KEY}"}), run)
    assert upstream.seen == []
    assert "error" in answer


def test_a_call_without_a_run_is_refused(api: TestClient, upstream: Upstream) -> None:
    """One process serves many runs, so an unlabelled call belongs to none of them."""
    answer = _post(api, _call("read", {"filePath": WORKDIR_FILE}), run=None)
    assert upstream.seen == []
    assert "error" in answer
...
def test_a_nested_argument_is_read_as_the_json_it_is(
    ...
    run = _open_run(api)
    _post(api, _call("bash", {"command": "uv sync", "extra": nested}), run)
    assert upstream.seen != []
...
def test_the_inbound_side_under_review_hands_the_result_back_whole(
    ...
    app = create_app(_table(settings, upstream), client, CollectingAuditSink())
    with TestClient(app=app) as reviewing:
        run = _open_run(reviewing)
        answer = _post(reviewing, _call("read", {"filePath": WORKDIR_FILE}), run)
...
def test_an_event_that_never_ends_is_cut_rather_than_passed_unread(
    ...
    monkeypatch.setattr("ads_supervisor.proxy.MAX_EVENT_BYTES", 64)
    upstream.stream = True
...
def test_the_run_header_is_ours_and_stops_here(api: TestClient, upstream: Upstream) -> None:
    run = _open_run(api)
    _post(api, _call("read", {"filePath": WORKDIR_FILE}), run)
...
def test_the_same_tool_on_another_server_is_a_different_tool(
    api: TestClient, upstream: Upstream
) -> None:
    """`read` is bound for the retriever only; the jira server's `read` binds to nothing."""
    run = _open_run(api)
    allowed = _post(api, _call("read", {"filePath": WORKDIR_FILE}), run, server="retriever")
    refused = _post(api, _call("read", {"filePath": WORKDIR_FILE}), run, server="jira")
...
def test_a_server_that_does_not_answer_is_a_bad_gateway(
    ...
    with TestClient(app=create_app(unreachable, policy_client, CollectingAuditSink())) as api:
...
def test_the_proxy_needs_no_token_of_its_own(api: TestClient, upstream: Upstream) -> None:
    """The agent is not made to authenticate to something it does not know exists."""
    run = _open_run(api)
    _post(api, _call("read", {"filePath": WORKDIR_FILE}), run)
    assert upstream.seen != []
