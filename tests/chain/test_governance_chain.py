...
from fake_mcp import create_app as create_fake_mcp
...
_EXAMPLE_DOCUMENT = msgspec.yaml.decode(
    (REPOSITORY / "charts" / "ads" / "policy.example.yaml").read_bytes(),
    type=dict[str, Any],
)
EXAMPLE_POLICY = load_policy(
    {**_EXAMPLE_DOCUMENT, "bindings": [*_EXAMPLE_DOCUMENT["bindings"], *policy_bindings()]}
)
...
class Chain:
    ...
    def start(self, stack: ExitStack) -> None:
        ...
        tools_url = stack.enter_context(served(create_fake_mcp()))
        ...
        guardrail_settings = GuardrailSettings(
            ...
            mcp_servers=(
                McpServer(VM_SITE, f"{tools_url}/mcp", KATA_VM),
                McpServer(CONTAINER_SITE, f"{tools_url}/mcp", APPLICATION_NODE),
            ),
    ...
    async def ask(
        ...
        streamer = ToolingChatStreamer(
            ToolSettings(
                mcp_servers=(VM_SITE, CONTAINER_SITE),
...
def test_a_permitted_call_goes_through_every_service(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel(
            [
                [tool_call("tools-vm__read_file", {"path": f"{WORKDIR}/src/app.py"})],
                [said("read it")],
            ]
        )
        deltas = await chain.ask(http, model)
        assert _notices(deltas) == []
        assert model.tool_result() == f"fake: contents of {WORKDIR}/src/app.py"
        assert "tools-container__read_file" in model.offered
...
def test_every_message_of_a_chat_shares_one_run(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        for _ in range(2):
            await chain.ask(
                http,
                ScriptedModel([[tool_call("tools-vm__echo", {"text": "hello"})], [said("ok")]]),
            )
...
def test_the_same_tool_is_decided_by_where_its_server_runs(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel(
            [
                [tool_call("tools-vm__run_command", {"command": "uv sync"}, "vm")],
                [tool_call("tools-container__run_command", {"command": "uv sync"}, "container")],
                [said("done")],
            ]
        )
        deltas = await chain.ask(http, model)
        assert model.tool_result(1) == "fake: would run uv sync"
        assert "refused" in model.tool_result(2)
        assert _notices(deltas) == [("tool-refused", "tools-container/run_command")]
...
def test_reading_outside_the_workdir_is_refused_and_journalled(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel(
            [[tool_call("tools-vm__read_file", {"path": "/etc/shadow"})], [said("no")]]
        )
        deltas = await chain.ask(http, model)
        assert _notices(deltas) == [("tool-refused", "tools-vm/read_file")]
...
def test_a_tool_nothing_binds_never_reaches_the_server(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel([[tool_call("tools-vm__diagnostics", {})], [said("no")]])
        deltas = await chain.ask(http, model)
        assert _notices(deltas) == [("tool-refused", "tools-vm/diagnostics")]
...
def test_a_secret_in_the_arguments_never_leaves(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel(
            [
                [tool_call("tools-vm__echo", {"text": f"key={FAKE_AWS_ACCESS_KEY}"})],
                [said("no")],
            ]
        )
        deltas = await chain.ask(http, model)
        assert _notices(deltas) == [("tool-refused", "tools-vm/echo")]
...
def test_a_secret_in_the_result_is_cut_out(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel([[tool_call("tools-vm__env_config", {})], [said("ok")]])
        deltas = await chain.ask(http, model)
        assert _notices(deltas) == []
        assert FAKE_AWS_ACCESS_KEY not in model.tool_result()
...
def test_by_default_an_injection_is_recorded_and_passed_on(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel([[tool_call("tools-vm__release_notes", {})], [said("ok")]])
        deltas = await chain.ask(http, model)
        assert _notices(deltas) == []
...
def test_an_enforced_injection_is_withheld_and_the_person_told_why(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel([[tool_call("tools-vm__release_notes", {})], [said("ok")]])
        deltas = await chain.ask(http, model)
        assert _notices(deltas) == [("prompt-injection", "tools-vm/release_notes")]
...
def test_an_injection_in_a_streamed_answer_is_withheld(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel(
            [
                [tool_call("tools-vm__tail_log", {"text": "streamed and clean"}, "clean")],
                [tool_call("tools-vm__tail_log", {"text": INJECTION_MARKER}, "poisoned")],
                [said("ok")],
            ]
        )
        deltas = await chain.ask(http, model)
        ...
        assert _notices(deltas) == [("prompt-injection", "tools-vm/tail_log")]
...
def test_a_chat_over_its_budget_loses_its_tools_and_only_that_chat(tmp_path: Path) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        ...
        await chain.ask(
            ...
            ScriptedModel(
                [
                    [tool_call("tools-vm__read_file", outside, "first")],
                    [tool_call("tools-vm__read_file", outside, "again")],
                    [said("stopped")],
                ]
            ),
        )
        ...
        blocked = ScriptedModel(
            [[tool_call("tools-vm__read_file", {"path": f"{WORKDIR}/a.py"})], [said("?")]]
        )
        assert _notices(await chain.ask(http, blocked)) == [("tool-refused", "tools-vm/read_file")]
        assert _rows(await chain.journal(), "conversation.revoked") != []

        elsewhere = ScriptedModel(
            [[tool_call("tools-vm__read_file", {"path": f"{WORKDIR}/a.py"})], [said("ok")]]
        )
...
@pytest.mark.parametrize("server", ["tools-vm", "tools-container"])
def test_both_servers_offer_the_same_tools(tmp_path: Path, server: str) -> None:
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
...
def test_the_real_classifier_withholds_the_planted_injection(tmp_path: Path) -> None:
    ...
    async def scenario(chain: Chain, http: aiohttp.ClientSession) -> None:
        model = ScriptedModel(
            [
                [tool_call("tools-vm__release_notes", {}, "poisoned")],
                [tool_call("tools-vm__read_file", {"path": f"{WORKDIR}/src/app.py"}, "clean")],
                [said("ok")],
            ]
        )
        deltas = await chain.ask(http, model)
        assert _notices(deltas) == [("prompt-injection", "tools-vm/release_notes")]
        assert model.tool_result(2) == f"fake: contents of {WORKDIR}/src/app.py"
